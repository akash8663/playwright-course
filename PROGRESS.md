...
| 7 | Authentication & State | failed | 1 | `.env`/dotenv wiring and env-var validation done correctly. Good test design (asserts blocked-without-auth, then works-with-reused-state) and found the real `context.setStorageState(path)` API (applies state to an existing context, not just at creation). BUT: split into two separate `test()` blocks with a hard file dependency and no `test.describe.serial` — reproduced a real intermittent failure under `fullyParallel: true` (5 reruns with the auth file deleted: failed on chromium once, `auth with storage state` ran before `auth` wrote the file). Needs either one merged test or `test.describe.serial`, exactly the gotcha flagged in the module 7 README. |
